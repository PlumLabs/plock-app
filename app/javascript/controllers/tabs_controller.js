...
export default class extends Controller {
  static classes = [ "activeTab", "inactiveTab", "activeTabLine" ]
  static targets = ["tab", "panel", "select", "tab"]
  static values = {
    index: 0,
  ...
  showTab() {
    this.panelTargets.forEach((panel, index) => {
      ...
      const activeTabLine = tab.querySelectorAll("span")[1]
      ...
      if (index === this.indexValue) {
        ...
        if (this.hasActiveTabClass) tab?.classList?.add(...this.activeTabClasses)
          ...
          // get the second span to set the active tab line color
        if (this.hasActiveTabLineClass) activeTabLine?.classList?.add(...this.activeTabLineClasses)
      } else {
        // the tab to hide
        ...
        // get the second span to remove the active tab line color
        if (this.hasActiveTabLineClass) activeTabLine?.classList?.remove(...this.activeTabLineClasses)
      }
    })
